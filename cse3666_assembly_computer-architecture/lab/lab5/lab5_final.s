...
buffer:	.space 4096		#allocate space for 1K words
	...
	.align 2
name:	.asciiz "CSE3666: Lab 5: ERIC WANG, KIERAN CLARKE (erw16108, kjc16113)\n"
...
main: 
	...
	move	$a0, $s0
	move	$a1, $s1
	jal	premerg_work 	# call merge_sort
...
merge_sort:
	...
	addi	$sp, $sp, -16		# EW: stack initialize
	
	move	$t0, $a1, 		# length of array --> $t0
	sll	$t0, $t0, 2		# multiply array elements by 4 --> $t0
	add	$a1, $a0, $t0		# store 
	...
	ble	$t0, 5, ms_end		# if array = 1 item, send to end
	...
	srl	$t0, $t0, 1		#reset our values
	sub	$a1, $a1, $a0		#reset our values		
	jal	merge_sort		# Bottom recursive call, first half will be split 
					# into individual array units ("single lists")
	
	lw	$a1, 4($sp)		# load end of array address from stack
	lw	$a0, 12($sp)		# load halfway array address from stack
	addi 	$a0, $a0, 1		#add one to middle of array, so if array is 8 elements, a0 is now 4+1 or 5 
...
ms_loop:				# could be of some use, not include thus far
