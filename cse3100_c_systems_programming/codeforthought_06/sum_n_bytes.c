//In this assignment, we wirte (lol typo) code to sum up the values of the first n bytes in a memory block
...
unsigned sum_n_bytes(unsigned *p, int n)
{
	...
	unsigned sum = 0; // initialize sum to 0
	
	for(int i = 0; i < n; i++){		// iteration begins at 0, increment by 1 up to "n"
		sum += *((unsigned char *) p + i); // per for loop iteration add the value from 0 to n
	}
	return sum;
}
