...
void * thread_main(void * arg_in)
{
    ...
    char buf[MSG_BUF_SIZE];		// char for buf of MSG_BUF_SIZE size like provided in client

        int max = gmn_get_max();
        snprintf(buf, MSG_BUF_SIZE, "%d\n", max);				// store max in array

        if (send_str(sockfd, buf) < 0) {						// send_str from demo code
                close(sockfd);									// socketio.c has implemented
                fprintf(stderr, "send_all returned -1.\n");		// a converter function from send_str
                exit(EXIT_FAILURE);								// to send_all with strlen(s) as
        }														// last parameter

        while(1){									// while true
        	if (recv_lines(sockfd, buf, MSG_BUF_SIZE) < 0) {
                close(sockfd);
                fprintf(stderr, "recv_lines failed.\n");
                exit(EXIT_FAILURE);
            }				// receive lines

        	int guess = atoi(buf);					// convert buffer string to int
        	int result = gmn_check(&gmn, guess);		// make result the number check
        	snprintf(buf, MSG_BUF_SIZE, "%d\n", result);
        	...
        	if(result == 0){						// if result is 0, exit while loop

        		// send_str(sockfd, msg);			// also works (the swap shouldn't matter)
        		break;
        	}
        }
    char * msg = gmn_get_message(&gmn);
    send_str(sockfd, msg);
    close(arg->sockfd);			// always close file descriptors post use
    free(arg);					// always free arg before returning NULL & exit... ?
