...
void * thread_player(void *arg_in)
{
    ...
    for (int i = 0; i < arg->n_rounds; i ++ ) {
        ...
        pthread_mutex_lock(&choice->mutex);		// lock	
        choice->status = S_INIT;				// set status
        choice->value = r;	
        //printf("_______ Player%d choice = %d\n", arg->id, r);
        //choice->status = S_INIT;				// set choice value to r
        pthread_cond_signal(&choice->cond);		// send choice r to ref
        choice->status = S_READY;
        pthread_mutex_unlock(&choice->mutex);
               					// change status to ready, may be redudnant 
        										// while(S_READY) is 1, while true
    	// while(choice->status != S_RESULT){							// while value is ready
    	//   	pthread_cond_wait(&choice->cond, &choice->mutex);	// wait for ref to return 
    	// }
        ...
        //printf("_______ Player%d outcome = %d\n", arg->id, outcome);
...
void * thread_referee(void *arg_in)
{
    ...
    for (int i = 0; i < arg->n_rounds; i ++) {
        ...
        pthread_mutex_lock(&p1->mutex);					// lock both mutexes
        ...
        outcome = compare_choices(choice1, choice2);
        // choice1->status = S_READY;
        // choice2->status = S_READY;
    	//printf("_&_&_&_&_ Referee Outcome = %d, \n", outcome);
