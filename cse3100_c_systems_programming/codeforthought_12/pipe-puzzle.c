...
int main(int argc, char *argv[])
{
    ...
    // EW: tests
    // printf("%d print cur start\n", cur);
    // printf("%ls print b array\n", b);
        ...
        for(int i = 0; i< n; i++)
        {
            ...
            {   
                ...
                else if(cur + a[cur] >= 0 && cur + a[cur] <n)
                {
					...
					// EW: if the current index + value of current index is >= 0
					// and if the current index + value of current index is < n
					// (in other words): otherwise we continue to find the solution: 
					// move either left or right the amount of index steps based on current index value
					
					// move only in the direction you can based on max index, either left or right
					
					// int rangeMax = 9;								// cannot exceed max range of a [0,9]
					// int nextMoveRight = cur + a[cur];				// ex. nextMoveRight = current index # + value at current index
									// ex. nextMoveLeft =  
					// write(pd[1], &nextMoveRight, sizeof(int));
					// printf("%d Before Moving\n", cur);
					
					cur += a[cur];				// set current index to cur_index + old index value
					b[moves] = cur;							// set b[moves (starting from 0)] = current value
					moves++;
                }
            }
            else
            {
                ...
                {
                    //Add your code here
					// EW: otherwise we continue to find the solution 
					// int nextMoveLeft = cur - a[cur];
