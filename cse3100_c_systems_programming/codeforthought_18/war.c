...
void* player_func(void* threadarg)
{
	...
	while(no_cards > 0 && no_cards + 2*tie_count < NUM_CARDS && rounds < MAX)
	{
		...
		no_cards--;							// decrement number of cards by 1 	
		pthread_barrier_wait(&barrier);			// wait for 2 player threads to line up
		...
		// need to wait a second time for hand
		//hand[no_cards--];
		pthread_barrier_wait(&barrier);		// Need to wait again for 2nd player to match win lose conditions / rerun the win, lose, tie if loop 
