...
#define		MIN_STACKSIZE				// define by Eric Wang
...
{
    // TODO Begin
    if(pstack->num_entries == stack_size(pstack)){		// if num_entries is maxed out at stacksize
		pstack->stack = realloc(pstack->stack, 4*STACKSIZE_STEP);		// increment by STACKSIZE_STEP
		pstack->num_entries = STACKSIZE_STEP;
    }
	else{					// if num_entries is 0, still increase by STACKSIZE_STEP
		pstack->stack = realloc(pstack->stack, 4*STACKSIZE_STEP);
		pstack->num_entries = STACKSIZE_STEP;
    	}
    pstack->sp++;						// incrementing the stack pointer
    (*pstack).stack[pstack->sp] = s;		// setting the top entry of stack to string "s"
}
...
int     stack_pop(stack_t *pstack, char **ps)			// has not been worked on
{
    // TODO Begin				DO NOT shrink stack
    if (pstack->sp < 0){		// stack is empty, return -1
    	return -1;
    }
    else{
    	**ps = pstack->stack[pstack->sp];
    	pstack->sp--;
    	
    	// if (pstack->num_entries > 3*stack->sp){		old code
...
{
    // TODO Begin						// not freeing pstack itself
    for(int i = 0; i < stack_size(pstack); i++){
    	free(pstack->sp);			// freeing each memory space used by strings starting from top
    	pstack->sp--;				// decrementing sp so as to reach sp == 0, to free all sps
    }
    free(pstack->stack);		// freeing the stack field
}
	// EW: Should mimic free_tokens from HW4, where it frees each index
	// EW: But should avoid the last line "free(t -> argv);	"
	// EW: Where it frees entire argv, likewise should not free entire pstack
...
void    process_line(stack_t * pstack, char *line)
{
    if (! strcmp(line, "i")) {
        ...
        // TODO Begin
        stack_pop(pstack, &s);
        if (stack_size(pstack) < 0){					// pop successful
        	error_message(ERR_EMPTY);
        }
        else{					// pop unsuccessful
        	printf("%s\n", s);
        }
