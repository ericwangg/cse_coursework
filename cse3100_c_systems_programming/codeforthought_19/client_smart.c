...
void guess_word(const char *scrambled, char *guess)
{
    ...
    string_reset(word_reset);			//EW: does qsort on scrambled, makes it alphabetical
    for(int i = 0; i<count; i++)
    {
		//fill in code below
		//find the first word in the dictionary that matches
		
		// for(int j = 0, i < MAX_WORD_LENGTH, j++){
					// no need to check character by character
		// }
		
		// EW: this won't work since exit
		//	merge scrambled fits in both merge and mergesort 
		// 
		
		//char dict_reset[MAX_WORD_LENGTH];		// buffer char array
		//char sr = string_reset(words[i]);		// string reset "qsort()" the 
