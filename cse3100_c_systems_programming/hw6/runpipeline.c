...
{
    Program *program = &programs[cur];	// dereference 
    pid_t myChild = fork();				// fork for child
     
	int progIn = program->fd_in;		// set abbr.
    int progOut = program->fd_out;
     
    if (myChild < 0){					// if forking unsuccessful
    	die("fork() failed");			// it's dead
    }
    else if (myChild == 0){				// forking successful
    	if(cur != num_programs - 1){
    		int myValue1 = dup2(progOut, 1);	// copy over write out value
    		if (myValue1 == -1){				// copying process has failed
    			die("dup2() failed");
    		}
    	}
    	if(cur > 0){
    		int myValue2 = dup2(progIn, 0);	// copy over read in value
    		if(myValue2 == -1){
    			die("dup2() failed");		// same copy failure		***TYPE DUPE2()***
    		}
    	}
    	for(int i = cur; i < num_programs; i++){
    		int progIn2 = programs[i].fd_in;		
    		int progOut2 = programs[i].fd_out;
    		
    		if (progIn2 > 0){
    			if(close(progIn2) < 0){			// if the current program file descrip. in closed value < 0
    				die("close() failed");			// coult not close
    			}
    		}
    		if(progOut2 != 1){
    			if(close(progOut2) < 0){			// if the current program file descrip. in closed value < 0
    				die("close() failed");			// could not close
    			}
    		}
    	}
    	execvp(program->argv[0], program->argv);			// returns when execvp fails
    	die("execvp() failed");								// and produces this error
    }
    else{
    	program->pid = myChild;						
    	if(progIn > 0 && cur > 0){					// if both current value > 0 and prog fd in > 0
    		if(close(progIn) < 0){					// close failure
    			die("close() failed");
    		}
    	}
    	if(progOut > 0 && cur != num_programs - 1){		// both prog Out > 0 and and cur index != # prog -1
    		if(close(progOut) < 0){							// closing the program fails 
    			die("close() failed");						// haha can't close, it's dead!
    		}
    	}
    }
    // TODO
...
int wait_on_program(Program *prog)
{
    ...
    // TODO
    int wait = waitpid(prog->pid, &exitStatus, 0);	// set wait conditions
    
    if (wait < 0){									// if that condition is less than 0
...
{
	int fd[2];					// pipe initialization
	
    for(int i = 1; i < num_programs; i++){			// counter start at 1, increments by 1 until no longer less than # of programs
    	int myPipe = pipe(fd);						// int gsives condition of pipe
    	if (myPipe != 0){								// if status anything besides 0
    		die("pipe() failed");					// dead pipe 
    	}
    	programs[i - 1].fd_out = fd[FD_STDOUT];				// set file descrptor write end "1"
    	programs[i].fd_in = fd[FD_STDIN];					// set fd read end "0" making the connection thru pipes
    }		
