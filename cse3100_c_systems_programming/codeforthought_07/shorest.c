...
t_pair *shortest_dist_pair(int n, t_point points[n])
{
	int i, j;
	double d;
	t_pair *p_pair;
	...
	double min_d = squared_dist(&(points[0]), &(points[1]));	// must use min_d
	p_pair -> p1 = points[0];
	p_pair -> p2 = points[1];
	...
	for(i = 0; i < n; i++){
		for(j = 0; j < n; j++){
			double sqd = squared_dist(&points[i], &points[j]);
			if(i == j){		// maybe this will work with le flip?
				;
			}
			else if(min_d > sqd){
				min_d = sqd;
				p_pair -> p1 = points[i];	// shortest dist pointer 1
				p_pair -> p2 = points[j];	// shortest dist pointer 2
			}
		}
	}
    return p_pair;
