...
{
    // TODO
    printer_t * pt = arg;			// make arg of printer_t structure 
    					// make new instance of job_queue_t structure
	
    while(pt->jq->njobs_fetched < pt->jq->njobs_max){	
    	//printf("id is %d, njobs is %d\n", pt->id, pt->njobs);
    	
    	
    	pthread_mutex_lock(&pt->jq->mutex);		// first lock the mutex 
    	
    	int jobsLeft = q_num_jobs(pt->jq);				
    	if(jobsLeft <= 0){
    		break;
    	}
    	
    	int jobFetched = q_fetch_job(pt->jq, pt->id);					// `
    	//printf("FETCHED JOBS %d\n\n", pt->jq->njobs_fetched);
    	
    	pthread_mutex_unlock(&pt->jq->mutex);		// then unlock the mutex for other jobs to use
    	
    	print_job(jobFetched);
    	pt->njobs++;
    }
    ...
    // for(int i = 0; i < DEFAULT_NUM_PRINTERS; i++){
    // 	for(int j = 0; j < DEFAULT_NUM_JOBS; j++){
    // 		pthread_mutex_lock(&jq.mutex);
    // 		int job = q_fetch_job(jq, j);
...
int main(int argc, char *argv[])
{
     ...
     *  */
     ...
     // Note: jq is an attribute of printer that is of structure printer_t, which has a attribute jq of structure jon_queue_t
    
    //pthread_mutex_init(&jq.mutex);
    
    
   // printer_t thread_args[num_printers];		// array to make thread arguments and ids
    pthread_t thread_id[num_printers];			// similar to mmul
    ...
    for(int i = 0; i < num_printers; i++){
    	...
    	printers[i].njobs = 0;
    	int printStatus = pthread_create(&thread_id[i], NULL, printer_main, &printers[i]);
    	if( printStatus){
    ...
    for(int j = 0; j < num_printers; j++){
    	int printStatus = pthread_join(thread_id[j], NULL);
    	if ( printStatus ){
	...
	pthread_mutex_destroy(&job_queue.mutex);		// should reference mutex in job_queue_t 
    q_destroy(&job_queue);
