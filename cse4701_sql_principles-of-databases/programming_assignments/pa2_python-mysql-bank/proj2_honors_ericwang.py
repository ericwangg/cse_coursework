    ...
    while True:
        ...
        #     break
            ...
            with connection.cursor() as cursor:
                ...
                acc_no = result.get('LAST_INSERT_ID()')
            with connection.cursor() as cursor:
                sql = "SELECT account_open_date FROM account WHERE account_no = %s;"
                ...
                if result == None:
                    ...
                    while True:
                        ...
                        if "-" in amount_text and amount_text.replace("-","").isnumeric() == True:
                            ...
                            # if amount < 10 and int(amount_text.replace(".","")) > 1000:
                            #     print("\n   Invalid. Decimal points exceeding 2 places\n")
                            if amount > acc_bal:
...
def summary(connection):
    ...
    with connection.cursor() as cursor:
        ...
        cursor.execute(sql)
        result = cursor.fetchone()
        accounts = result.get('COUNT(*)')
    print("\nBeginning summary calculations...\n")
    for account in range(accounts):
        with connection.cursor() as cursor:
            sql = "SELECT * FROM account ORDER BY account_no LIMIT %s, 1;"
            cursor.execute(sql, account)
            result = cursor.fetchone()
            name = result.get('name_on_account')
            print(f"   Reading Account {name} ... \n")
            time.sleep(5)  # Delay instructions per "part vi" of PDF
            sub_sum = result.get('balance')
            sum += sub_sum
            print(f"   Account {name} has been read\n")
            time.sleep(30)   # it's more crucial to have 10 seconds after read to achieve part 3 of PDF
        connection.commit()
    with connection.cursor() as cursor:
        sql = "INSERT INTO bank_summary (total_amount) VALUES (%d);" % (sum)
        cursor.execute(sql)
    with connection.cursor() as cursor:
        sql = "SELECT LAST_INSERT_ID();"
        cursor.execute(sql)
        result = cursor.fetchone()
        sn = result.get('LAST_INSERT_ID()')
    with connection.cursor() as cursor:
        sql = "SELECT * FROM bank_summary WHERE sn = %s;"
        ...
        print(f"|Total Amount in Bank: {sum}")
        print(f"|Timestamp: {timestamp}")
        print("+--------------------------\n")
    connection.commit()

def transfer_t1(connection):
    # Require 3+ while loops here to control when inputs break out and return to menu
    loop_1 = True
    loop_2 = True
    while loop_1 == True:
